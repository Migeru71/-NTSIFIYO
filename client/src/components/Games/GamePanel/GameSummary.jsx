...
const GameSummary = ({
    ...
    useEffect(() => {
        const calculateResults = async () => {
            try {
                // If currentGameData is present, it contains the true session activityId.
                // The prop "activityId" passed by games is actually the gameId from the URL params.
                ...
                const payload = {
                    ...
                    responseLogs: (responseLogs || []).map(log => ({
                        ...
                        responseAnswerId: log.answerId, // Backend expects responseAnswerId
    ...
    const isPairsGame = responseLogs && responseLogs.length > 0 && responseLogs.every(log => log.questionId === null);

    const pairsSummary = useMemo(() => {
        if (!isPairsGame) return null;
        ...
        responseLogs.forEach(log => {
            if (!wordsMap.has(log.answerId)) {
                ...
                if (log.isCorrect) {
                    wordsMap.get(log.answerId).isCorrect = true;
                }
            }
        });
        const completed = [];
    ...
    return (
        <div className="game-summary-container">
            <div className="gs-card">
                ...
                <div className="gs-header gs-header-centered">
                    <h2>¡Actividad Completada!</h2>
                </div>

                {/* Content Row for Desktop (Columns for Mobile) */}
                        ...
                        <div className="gs-metrics-section">
                            <div className="gs-metric" title={`${percentage}% de precisión`}>
                                ...
                                <div className="gs-metric-stars">
                                    {[...Array(5)].map((_, i) => (
                                        <span key={i} className={`gs-star ${i < starsCount ? 'filled' : 'empty'}`}>
                                            ★
                                        </span>
                                    ))}
                                </div>
                        ...
                        <div className={`gs-logs-section ${expandedLogs ? 'expanded' : ''}`}>
                            ...
                            <div className="gs-logs-list">
                                {isPairsGame && pairsSummary ? (
                                    <div className="gs-pairs-summary" style={{ display: 'flex', gap: '20px', justifyContent: 'center', textAlign: 'left', padding: '10px 0' }}>
                                        <div className="gs-pairs-completed" style={{ background: '#dcfce7', padding: '15px', borderRadius: '12px', flex: 1 }}>
                                            <h4 style={{ color: '#166534', marginTop: 0, marginBottom: '10px' }}>✅ Terminadas</h4>
                                            <ul style={{ paddingLeft: '20px', margin: 0, color: '#15803d', fontWeight: 'bold' }}>
                                                {pairsSummary.completed.map((word, i) => <li key={i}>{word}</li>)}
                                                {pairsSummary.completed.length === 0 && <li>Ninguna</li>}
                                            </ul>
                                        </div>
                                        <div className="gs-pairs-incomplete" style={{ background: '#fee2e2', padding: '15px', borderRadius: '12px', flex: 1 }}>
                                            <h4 style={{ color: '#991b1b', marginTop: 0, marginBottom: '10px' }}>❌ No Terminadas</h4>
                                            <ul style={{ paddingLeft: '20px', margin: 0, color: '#b91c1c', fontWeight: 'bold' }}>
                                                {pairsSummary.incomplete.map((word, i) => <li key={i}>{word}</li>)}
                                                {pairsSummary.incomplete.length === 0 && <li>Ninguna</li>}
                                            </ul>
                                        </div>
                                    </div>
                                ) : (
                ...
                <div className="gs-actions gs-actions-centered">
                    {onRetry && (
